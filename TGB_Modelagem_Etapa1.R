...
ETAPA1$log_NumMorte2 = log(ETAPA1$NumMort+1)
...
#
#
# [1] "Data"          "Estacao"       "Turno"         "TempoEsp"     
#[5] "TempExt"       "URExt"         "EntExt"        "NumAves"      
#[9] "NumMort"       "Morte"         "TotalAves"     "log_TotalAves"
...
#Round = 6	alfa = 0.0006842467	Value = -966.5766 

opt_mob1$Best_Par

##### Ajustando o modelo

mob1 = glmtree(NumMort ~ TempoEsp|
                 TempExt+URExt+ 
                 Estacao+Turno,
               data = dados_treino,
               family = poisson(link = "log"),
...
#                 size = 1.5) +
#   stat_function(fun = fun_curvas_mob1, args = list(i=3),
#                 aes(colour = rownames(coef_mob1)[3]), 
#                 size = 1.5)+
#   stat_function(fun = fun_curvas_mob1, args = list(i=4),
#                 aes(colour = rownames(coef_mob1)[4]), 
#                 size = 1.5)+
...
ggsave(filename = "ETAPA1_Mod_MOB1.png", 
...
riscos$MOB_1 = c(calcula_EQM_e_IC(predito_mob1,
                                  dados_teste$NumMort,
...
NumMort ~ TempoEsp| 
  TempExt+URExt+Estacao+Turno

start_time <- Sys.time()
MOB_2 = glmtree(NumMort ~ TempoEsp| 
                  TempExt+URExt+Estacao+Turno,
               data = dados_treino,
               family = poisson(link = "log"),
               offset = log_TotalAves,
...
g_MOB_2=
  ...
  geom_node_label(# map content to label for each line
    ...
    # set graphical parameters for each line in same order
    line_gpar = list(list(size = 6, parse = T),
                     list(size = 6, parse = T)),
    ids = "terminal",
    # nudge labels towards bottom so that edge labels have enough space
    # alternatively use shift argument of edge_label
...
ggsave(filename = "ETAPA1_Mod_MOB2.png", 
       path ="D:/Mega/ufscar_mega/Ufscar/TG/Apliacao/Etapa1/Figuras",  
       width = 30.75, 
       height = 15.5, 
       units = "cm")
...
##### Predizendo
predito_mob2 = predict(MOB_2 ,dados_teste)
...
CART_3 = rpart(matrix(c(TotalAves,NumMort),
                      ncol=2,
                      byrow=F) ~ 
                 TempoEsp+TempExt+URExt+Estacao+Turno,
...
CART_3_obs_por_no = 
  split(dados_treino,
        predict(as.party(CART_3), type = "node"))
...
dados_CART_3 = ggparty(as.party(CART_3))
dados_CART_3$data = 
...
ggsave(filename = "ETAPA1_Mod_CART3.png", 
       path ="D:/Mega/ufscar_mega/Ufscar/TG/Apliacao/Etapa1/Figuras",  
       width = 30.75, 
       height = 15.5, 
       units = "cm")
...
predito_CART_3 = predict(CART_3,dados_teste)*dados_teste$TotalAves
...
# 631.17830 --> sem offset
# 628.186890 --> com offset
...
CTREE_1 = ctree(NumMort ~ TempoEsp+TempExt+URExt+Estacao+Turno+TotalAves,
                data= dados_treino,
...
ggsave(filename = "ETAPA1_Mod_CTREE1.png", 
       path ="D:/Mega/ufscar_mega/Ufscar/TG/Apliacao/Etapa1/Figuras",  
       width = 30.75, 
       height = 15.5, 
       units = "cm")
...
##### Predizendo
predito_CTREE_2 = exp( predict(CTREE_2,dados_teste) )-1
...
opt_floresta1$Best_Par


floresta_1 = 
  ranger(x = dados_treino[,c(2:6,11)],
...
graf_flor_impor1 = 
  ...
  theme(axis.text.y = element_text(face="bold", color="black", 
                                   size=15)) +
  labs(title = "Floresta Aleatória 1", 
...
ggsave(filename = "ETAPA1_Mod_RF1_Importancia.png", 
       path ="D:/Mega/ufscar_mega/Ufscar/TG/Apliacao/Etapa1/Figuras",  
       width = 30.75, 
       height = 15.5, 
       units = "cm")
...
cv_floresta2 = function(num_arvore,num_no,k=5,s=777){
  ...
  for(i in 1:k){
    ...
    floresta = 
      ranger(x = trainData[,c(2:6,11)],
             y = trainData$log_NumMorte2,
             num.trees = num_arvore,
             min.node.size = num_no)
    
    predito_floresta =
      exp(predict(floresta,testData)$predictions)-1
...
opt_floresta2$Best_Par


floresta_2 = 
  ranger(x = dados_treino[,c(2:6,11)],
         y = dados_treino$log_NumMorte2,
...
graf_flor_impor2 = 
  ...
  theme(axis.text.y = element_text(face="bold", color="black", 
                                   size=15)) +
  labs(title = "Floresta Aleatória 2", 
       x = "", y = "Importancia da Variável")

graf_flor_impor2

ggsave(filename = "ETAPA1_Mod_RF2_Importancia.png", 
       path ="D:/Mega/ufscar_mega/Ufscar/TG/Apliacao/Etapa1/Figuras",  
       width = 30.75, 
       height = 15.5, 
       units = "cm")
...
predito_floresta_2 = 
  exp(predict(floresta_2,dados_teste)$predictions)-1
...
xgb_mat_treino = 
  xgb.DMatrix(model.matrix(NumMort~.,data=dados_treino[,c(2:6,9)])[,-1],
              label = dados_treino$NumMort)

xgb_mat_teste = 
  xgb.DMatrix(model.matrix(NumMort~.,data=dados_teste[,c(2:6,9)])[,-1],
...
xgb_mat_treino2 = 
  xgb.DMatrix(model.matrix(NumMort~.,data=dados_treino[,c(2:6,9)])[,-1],
              label = dados_treino$log_NumMorte2)

xgb_mat_teste2 = 
  xgb.DMatrix(model.matrix(NumMort~.,data=dados_teste[,c(2:6,9)])[,-1],
              label = dados_teste$log_NumMorte2)
...
predito_xgb_2 = 
  exp(predict(modelo_xgb_2,xgb_mat_teste2))-1
...
ggsave(filename = "ETAPA1_Mod_Importancia_ML.png", 
       path ="D:/Mega/ufscar_mega/Ufscar/TG/Apliacao/Etapa1/Figuras",  
       width = 30.75, 
       height = 15.5, 
       units = "cm")
...
glm_1 = glm(NumMort ~ TempoEsp+TempExt+URExt+Estacao+Turno,
            data= dados_treino,
            family = poisson(link = "log"),
            offset = log_TotalAves)
...
glm_2 = glm(NumMort ~ TempoEsp+TempExt+URExt+Turno*Estacao,
            data= dados_treino,
            family = poisson(link = "log"),
            offset = log_TotalAves)
...
round(resumo_glm_2$coefficients,digits=)
...
xtable(data.frame(matrix(unlist(riscos), nrow=length(riscos), byrow=TRUE)))
